...
#
# Correções para Android/Bionic
#

COMPLETE_C="src/frontend/parser/complete.c"
SNP2VA_C="src/frontend/snp2va.c"

if [[ ! -f "$COMPLETE_C" ]]; then
    echo "Arquivo não encontrado: $COMPLETE_C" >&2
    exit 1
fi

if [[ ! -f "$SNP2VA_C" ]]; then
    echo "Arquivo não encontrado: $SNP2VA_C" >&2
    exit 1
fi

echo "==> Aplicando correções para Android/Bionic"

# Android/Bionic não fornece as funções usadas para enumerar usuários.
sed -i -E \
    -e 's/\(void\)[[:space:]]*setpwent[[:space:]]*\(\);/(void)0;/g' \
    -e 's/pw[[:space:]]*=[[:space:]]*getpwent[[:space:]]*\(\)/pw = NULL/g' \
    -e 's/\(void\)[[:space:]]*endpwent[[:space:]]*\(\);/(void)0;/g' \
    "$COMPLETE_C"

# Verifica se ainda existem chamadas, desconsiderando possíveis comentários
# e declarações encontradas por grep.
if grep -nE '(getpwent|setpwent|endpwent)[[:space:]]*\(\)' \
    "$COMPLETE_C"; then
    echo "Erro: funções incompatíveis ainda existem em $COMPLETE_C" >&2
    exit 1
fi

# snp2va.c utiliza cpow(), cuja declaração está em complex.h.
if ! grep -qE \
    '^[[:space:]]*#[[:space:]]*include[[:space:]]*<complex\.h>' \
    "$SNP2VA_C"; then
    sed -i '1i#include <complex.h>' "$SNP2VA_C"
fi

if ! grep -qE \
    '^[[:space:]]*#[[:space:]]*include[[:space:]]*<complex\.h>' \
    "$SNP2VA_C"; then
    echo "Erro: não foi possível adicionar <complex.h> em $SNP2VA_C" >&2
    exit 1
fi
